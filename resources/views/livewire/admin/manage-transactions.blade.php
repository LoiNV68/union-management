    ...
    <div class="premium-card p-6 mb-6">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-neutral-900 dark:text-neutral-100">{{ __('Quản lý Thu Chi') }}
                    </h1>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <div class="col-span-1">
                                <flux:radio.group wire:model="type" :label="__('Loại')" required>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" wire:click="closeViewModal">
            <div class="w-full max-w-4xl rounded-lg bg-white dark:bg-neutral-800 p-6 shadow-xl max-h-[90vh] overflow-y-auto"
                ...
                <div class="mb-4 flex gap-4">
                    ...
                    <div class="w-[200px]">
                        <flux:select wire:model.live="classSearch" placeholder="Lọc theo lớp">
                ...
                <div class="mt-6 flex items-center justify-between">
                    <div>
                        @if ($viewingTransaction->status == 0)
                            <flux:button wire:click="closeTransaction({{ $viewingTransaction->id }})" variant="danger">
                                {{ __('Đóng khoản thu') }}
                            </flux:button>
                        @else
                            <flux:button wire:click="openTransaction({{ $viewingTransaction->id }})" variant="primary">
                                {{ __('Mở lại khoản thu') }}
                            </flux:button>
                        @endif
                    </div>
            ...
            <div class="premium-card p-5 hover:shadow-lg transition-shadow">
                <div class="flex items-start justify-between">
                    ...
                    <div class="flex items-center gap-1">
                        ...
                        @else
                            <flux:button wire:click="openTransaction({{ $transaction->id }})" variant="ghost" size="sm"
                                title="Mở lại">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
